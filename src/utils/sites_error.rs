...
impl ResponseError for SitesError{
    fn error_response(&self) -> actix_web::HttpResponse<actix_web::body::BoxBody> {
        match self{
            ...
            SitesError::SitesDatabaseError(db_err) => HttpResponse::InternalServerError().json(ErrResponse{
                error: "Database error ".to_string(),
                message: format!(":{}", db_err),
            }),
            SitesError::SitesDatabaseConnectionError(runtime_err) => HttpResponse::InternalServerError().json(ErrResponse{
                error: "Database Connection error ".to_string(),
                message: format!(":{}", runtime_err),
            }),
            SitesError::SitesInternalServerError(err) => HttpResponse::InternalServerError().json(ErrResponse{
                error: "Internal server error ".to_string(),
                message: format!(":{}", err),
            }),
            SitesError::SitesQueryError(runtime_err) => HttpResponse::InternalServerError().json(ErrResponse{
                error:"Query Error".to_string(),
                message: format!(":{}",runtime_err)
            }),
            SitesError::SitesValidationError(validation_errors) => HttpResponse::NotAcceptable().json(ErrResponse{
    ...
    fn status_code(&self) -> actix_web::http::StatusCode {
        match self{
            ...
            SitesError::SitesDatabaseConnectionError(runtime_err) => StatusCode::INTERNAL_SERVER_ERROR,
